...
### Fixed
- Fixed `useAppendTransform` plugin incorrectly transforming jQuery `.append()` calls
  - Now skips `$(...).append()`, `jQuery(...).append()`, `$el.append()`, and chained jQuery calls like `$(...).find(...).append()`

## [2.8.2] - 2026-02-23

### Fixed
- Fixed `useAppendTransform` plugin crash when `.append()` receives non-string primitives (numbers, booleans, etc.)
  - `element.append(3)` was incorrectly transformed to `element.appendChild(3)` causing `TypeError: 3.appendChild is not a function`
  - Now wraps all primitive literals with `document.createTextNode()` and adds runtime `instanceof Node` check for variables

## [2.8.1] - 2026-02-20

### Fixed
- Removed dead code: `Promise.all(promises.filter((p) => p instanceof Promise))` in `processPattern` (promises array never contains Promise instances)
- Fixed source map file path generation using `String.replace(string)` → `String.replace(RegExp)` to correctly handle paths containing `.js` or `.css` in directory names
